...
export class BusinessUnitService {
    ...
    const [businessUnits, total] = await Promise.all([
      prisma.business_units.findMany({
        ...
        include: {
          ...
          _count: { select: { users_users_business_unit_idTobusiness_units: true } },
        },
        orderBy: { [sort_by]: sort_order },
      }),
      prisma.business_units.count({ where }),
    ]);
  ...
  async create(data: CreateBusinessUnitData, userId: string) {
    ...
    const code = data.code?.trim().toUpperCase() || name.toUpperCase().replace(/\s+/g, '_');
