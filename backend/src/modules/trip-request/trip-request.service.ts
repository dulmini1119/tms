...
export const createTripRequest = async (data: any) => {
  ...
  const workflowUsers: Array<{ userId: string; role: "MANAGER" | "HOD" }> = [];

  let managerUser =
    requester?.departments_users_department_idTodepartments?.business_units
      ?.users_business_units_head_idTousers;
  let hodUser =
    requester?.departments_users_department_idTodepartments
      ?.users_departments_head_idTousers;
  ...
  const newRequest = await prisma.$transaction(async (tx) => {
    const created = await tx.trip_requests.create({
      data: {
        ...
        ac_required:
          requirements.acRequired !== undefined ? requirements.acRequired : true,
        ...
        approval_required:
          rest.approvalRequired !== undefined ? rest.approvalRequired : true,
