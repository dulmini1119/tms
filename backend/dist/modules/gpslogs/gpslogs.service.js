...
export class GPSLogsService {
     ...
     */
    async getGPSLogs(filters) {
        const { page = 1, limit = 10, searchTerm, status = 'all', vehicleId, driverId, } = filters;
        const whereClause = {};
        if (vehicleId)
     ...
     */
    async getTripReplayData(tripAssignmentId) {
        ...
        for (let i = 1; i < logs.length; i++) {
            const prev = logs[i - 1];
            const curr = logs[i];
            totalDistance += haversineDistance(toNumber(prev.latitude), toNumber(prev.longitude), toNumber(curr.latitude), toNumber(curr.longitude));
        }
...
//# sourceMappingURL=gpslogs.service.js.map