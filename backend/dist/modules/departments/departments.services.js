...
export class DepartmentService {
    ...
    async create(data, userId) {
        ...
        if (!name) {
            throw new AppError(ERROR_CODES.BAD_REQUEST, "Department name is required", HTTP_STATUS.BAD_REQUEST);
        }
    ...
    async delete(id) {
        ...
        if (counts.users_users_department_idTodepartments > 0 ||
            counts.vehicles > 0 ||
            counts.expenses > 0 ||
            counts.trip_requests > 0) {
            throw new AppError(ERROR_CODES.FORBIDDEN, "Cannot delete department with associated users, vehicles, expenses, or trip requests", HTTP_STATUS.FORBIDDEN);
        }
        await prisma.departments.delete({ where: { id } });
        return true;
    }
    // --- GET POTENTIAL DEPARTMENT HEADS ---
    async getPotentialHeads(page = 1, limit = 10) {
        const skip = (page - 1) * limit;
