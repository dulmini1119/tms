...
export class VehiclesService {
    ...
    async createVehicle(data, userId) {
        ...
        if (exists) {
            throw new AppError(ERROR_CODES.ALREADY_EXISTS, "Vehicle already exists", HTTP_STATUS.CONFLICT);
        }
    ...
    async getVehicles(filter) {
        ...
        if (filter) {
            ...
            if (filter.vehicle_type)
                where.vehicle_type = filter.vehicle_type;
            if (filter.availability_status)
                where.availability_status = filter.availability_status;
            if (filter.cab_service_id)
                where.cab_service_id = filter.cab_service_id;
            // FIX: Corrected object syntax for Prisma 'mode: insensitive'
            if (filter.ownership_type)
                where.ownership_type = { equals: filter.ownership_type, mode: "insensitive" };
        }
    ...
    async getVehicleById(id) {
        ...
        if (!vehicle) {
            throw new AppError(ERROR_CODES.NOT_FOUND, "Vehicle not found", HTTP_STATUS.NOT_FOUND);
        }
        return {
            ...vehicle,
    ...
    async updateVehicle(id, data, userId) {
        ...
        if (data.registration_number) {
            ...
            if (exists) {
                throw new AppError(ERROR_CODES.ALREADY_EXISTS, "Registration number already in use", HTTP_STATUS.CONFLICT);
            }
        }
