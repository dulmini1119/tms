...
export class BusinessUnitService {
    async findOrThrow(id) {
        ...
        if (!bu) {
            throw new AppError(ERROR_CODES.NOT_FOUND, "Business Unit not found", HTTP_STATUS.NOT_FOUND);
        }
        return bu;
    }
    async getAll(query = {}) {
        let { page = 1, limit = 10, search = "", sort_by = "created_at", sort_order = "desc", } = query;
        // FIX: convert to numbers
        const pageNum = Number(page);
        const limitNum = Number(limit);
        if (isNaN(pageNum) || isNaN(limitNum)) {
            throw new AppError(ERROR_CODES.VALIDATION_ERROR, "page and limit must be valid numbers", HTTP_STATUS.BAD_REQUEST);
        }
    ...
    async update(id, data, userId) {
        ...
        if (checkUniqueness.length > 0) {
            ...
            if (existing) {
                throw new AppError(ERROR_CODES.ALREADY_EXISTS, "Another Business Unit exists with the same name or code", HTTP_STATUS.CONFLICT);
            }
        }
    ...
    async delete(id) {
        ...
        if (bu._count.users_users_business_unit_idTobusiness_units > 0) {
            throw new AppError(ERROR_CODES.FORBIDDEN, "Cannot delete Business Unit that has employees assigned", HTTP_STATUS.FORBIDDEN);
        }
