 ...
 * Create cab service
 */
export const createCabService = async (req, res) => {
    ...
    try {
        ...
        const service = await CabServicesService.create(value, req.user.id);
 ...
 * List cab services
 */
export const listCabServices = async (req, res) => {
    try {
...
export const updateCabService = async (req, res) => {
    try {
        ...
        // --------------------
        const service = await CabServicesService.update(req.params.id, value, // <-- Use the validated value
        req.user.id);
        res.json(service);
    }
    catch (error) {
...
//# sourceMappingURL=cab-service.controller.js.map