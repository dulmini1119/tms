 ...
 * PREVIEW
 */
export const previewInvoice = async (cabServiceId, month, userId) => {
    ...
    const totalAmount = trips.reduce((sum, t) => sum + Number(t.total_cost), 0);
    return {
 ...
 * GENERATE
 */
export const generateInvoice = async (invoiceId, dueDate, notes) => {
    ...
    const trips = await repo.getUninvoicedTrips(invoice.cab_service_id, invoice.billing_month);
    const totalAmount = trips.reduce((sum, t) => sum + Number(t.total_cost), 0);
 ...
 * LIST
 */
export const listInvoices = async () => prisma.invoice.findMany({
    include: {
