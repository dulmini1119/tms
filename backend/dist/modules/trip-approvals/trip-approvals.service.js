...
const calculateFinalStatus = (approvals) => {
...
const calculateCurrentLevel = (approvals) => {
...
const mapApprovalToFrontend = (request, approvals) => {
...
export const getAllTripApprovals = async (filters) => {
    ...
    const transformedData = requests.map((req) => mapApprovalToFrontend(req, req.trip_approvals || []));
    ...
    return {
        ...
        meta: {
            ...
            page,
            pageSize,
            totalPages: Math.ceil((status === "all" ? totalCount : filteredData.length) / pageSize),
        },
    };
};
...
export const processApprovalAction = async (approvalId, payload) => {
...
//# sourceMappingURL=trip-approvals.service.js.map