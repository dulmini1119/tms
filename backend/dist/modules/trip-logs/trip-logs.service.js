...
const getDistanceSourceForAssignment = (gpsRows) => {
    if (!gpsRows.length)
        return null;
    if (gpsRows.length < 2)
        return "GPS (insufficient points)";
...
const attachDistanceSource = async (logs) => {
    const assignmentIds = Array.from(new Set(logs
    ...
    return logs.map((log) => {
        const source = log.trip_assignment_id && grouped.has(log.trip_assignment_id)
            ? getDistanceSourceForAssignment(grouped.get(log.trip_assignment_id) || [])
            : null;
        return { ...log, distance_source: source };
    });
};
export const getAllTripLogs = async ({ searchTerm, status, page, pageSize, startDate, endDate, }) => {
...
export const updateTripLog = async (id, data) => {
    ...
    // 1. Handle Status
    if (data.tripStatus)
        updatePayload.trip_status = data.tripStatus;
    // 2. Handle Metrics
    if (data.actualDistance !== undefined)
        updatePayload.actual_distance = data.actualDistance;
    // 3. Handle Times & Duration Calculation
    if (data.actualDeparture)
        updatePayload.actual_departure = new Date(data.actualDeparture);
    if (data.actualArrival)
        updatePayload.actual_arrival = new Date(data.actualArrival);
    ...
    if (data.totalCost !== undefined)
        updatePayload.total_cost = data.totalCost;
    if (data.fuelCost !== undefined)
        updatePayload.fuel_cost = data.fuelCost;
    if (data.tollCharges !== undefined)
        updatePayload.toll_charges = data.tollCharges;
    if (data.parkingCharges !== undefined)
        updatePayload.parking_charges = data.parkingCharges;
    if (data.otherCharges !== undefined)
        updatePayload.other_charges = data.otherCharges;
    if (data.currency)
        updatePayload.currency = data.currency;
    // 5. Handle Ratings (Matches Schema Columns)
    if (data.overallRating !== undefined)
        updatePayload.overall_rating = data.overallRating;
    if (data.punctualityRating !== undefined)
        updatePayload.punctuality_rating = data.punctualityRating;
    if (data.driverBehaviorRating !== undefined)
        updatePayload.driver_behavior_rating = data.driverBehaviorRating;
    if (data.vehicleConditionRating !== undefined)
        updatePayload.vehicle_condition_rating = data.vehicleConditionRating;
    // 6. Other fields
    if (data.onTime !== undefined)
        updatePayload.on_time = data.onTime;
...
export const generateTripLogCsv = async (filters) => {
...
//# sourceMappingURL=trip-logs.service.js.map