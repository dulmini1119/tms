...
const calculateAssignmentDistanceKm = async (assignmentId) => {
    ...
    for (let i = 1; i < gps.length; i++) {
        total += haversineDistanceKm(toNumber(gps[i - 1].latitude), toNumber(gps[i - 1].longitude), toNumber(gps[i].latitude), toNumber(gps[i].longitude));
    }
    return round2(total);
};
const mapDriverAssignment = (assignment) => {
    ...
    return {
        ...
        status,
        distance: assignment.actual_distance != null
...
const isManagerPosition = (position) => (position || "").toUpperCase().includes("MANAGER");
const mapApprovalQueueItem = (approval) => {
    const trip = approval.trip_requests;
    const requester = trip?.users_trip_requests_requested_by_user_idTousers;
    const department = requester?.departments_users_department_idTodepartments?.name || "N/A";
    return {
        ...
        employee: [requester?.first_name, requester?.last_name].filter(Boolean).join(" ").trim() ||
            "Unknown",
...
export class PortalService {
    ...
    static async getTeamApprovalQueue(userId) {
        const user = await prisma.users.findUnique({
            where: { id: userId },
            include: {
                departments_users_department_idTodepartments: { select: { name: true } },
    ...
    static async processTeamApproval(userId, approvalId, payload) {
        ...
        if (step.approver_user_id !== userId)
            throw new Error("Approval step is not assigned to this user");
        if (step.status !== "Pending")
            throw new Error("Approval already processed");
        const previousSteps = step.trip_requests.trip_approvals?.filter((s) => s.approval_level < step.approval_level) || [];
        const canAct = previousSteps.every((s) => s.status === "Approved");
        if (!canAct)
            throw new Error("Previous approval level is still pending");
        const now = new Date();
    ...
    static async getVehicleAdminDashboard(userId) {
        const user = await prisma.users.findUnique({
            where: { id: userId },
            include: {
                departments_users_department_idTodepartments: { select: { name: true } },
        ...
        if (!user)
            throw new Error("User not found");
        const [availableVehicles, assignedVehicles, activeTrips, approvedWaiting, pendingTrips, fleet] = await Promise.all([
        ...
        return {
            ...
            pendingTrips: pendingTrips.map((trip) => ({
                ...
                employee: [
                    ...
                    .join(" ")
                    .trim() || "Unknown",
                department: trip.users_trip_requests_requested_by_user_idTousers
                    ?.departments_users_department_idTodepartments?.name || "N/A",
                ...
                approvedBy: trip.trip_approvals?.[0]?.users
            ...
            fleet: fleet.map((v) => ({
                ...
                driver: v.drivers_vehicles_current_driver_idTodrivers
                    ? `${v.drivers_vehicles_current_driver_idTodrivers.users_drivers_user_idTousers
                        ?.first_name || ""} ${v.drivers_vehicles_current_driver_idTodrivers.users_drivers_user_idTousers
                        ?.last_name || ""}`.trim()
                    : "Unassigned",
            })),
        };
    }
    static async getVehicleAdminProfile(userId) {
        const user = await prisma.users.findUnique({
            where: { id: userId },
            include: {
                departments_users_department_idTodepartments: { select: { name: true } },
        ...
        if (!user)
            throw new Error("User not found");
        const [totalAssignments, successfulAssignments, pendingAssignments, activeVehicles] = await Promise.all([
    ...
    static async getVehicleAdminAssignments() {
        ...
        await Promise.all(driverUsers.map((u) => prisma.drivers.upsert({
            where: { user_id: u.id },
            update: {
                deleted_at: null,
                updated_at: new Date(),
            ...
            create: {
                user_id: u.id,
                license_number: `AUTO-${u.employee_id || u.id.slice(0, 8)}-${u.id.slice(0, 4)}`.slice(0, 100),
        ...
        const assignedByRequest = new Map(assignments.map((a) => [a.trip_request_id, a]));
        const trips = approvedTrips.map((trip) => {
            ...
            return {
                ...
                assignedDriver: assignment.drivers
                    ? `${assignment.drivers.users_drivers_user_idTousers?.first_name || ""} ${assignment.drivers.users_drivers_user_idTousers?.last_name || ""}`.trim()
                    : null,
            };
        });
        const driverOptionsFromDrivers = drivers.map((d) => ({
            id: d.id,
            name: `${d.users_drivers_user_idTousers?.first_name || ""} ${d.users_drivers_user_idTousers?.last_name || ""}`.trim(),
            ...
            vehicles: vehicles.map((v) => ({
                ...
                driver: v.drivers_vehicles_current_driver_idTodrivers
                    ? `${v.drivers_vehicles_current_driver_idTodrivers.users_drivers_user_idTousers
                        ...
                        ?.last_name || ""}`.trim()
                    : "Unassigned",
                driverPhone: v.drivers_vehicles_current_driver_idTodrivers?.users_drivers_user_idTousers
                    ?.phone || "",
                status: (v.availability_status || "Unknown").toLowerCase(),
                location: v.current_location || "N/A",
                fuelLevel: 0,
    ...
    static async assignVehicleAdminTrip(userId, payload) {
        ...
        if (!resolvedDriver) {
            ...
            const autoLicense = `AUTO-${base}-${Date.now().toString().slice(-6)}`.slice(0, 100);
