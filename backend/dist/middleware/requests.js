...
export const restrictEditAfterApproval = async (req, res, next) => {
    const { id } = req.params;
    try {
        ...
        if (!trip.status || !EDITABLE_STATUSES.has(trip.status)) {
            return ApiResponse.error(res, 'FORBIDDEN', `Cannot edit trip request in status "${trip.status}". ` +
                'Only Pending requests can be modified.', 403);
        }
        next();
    }
    catch (error) {
