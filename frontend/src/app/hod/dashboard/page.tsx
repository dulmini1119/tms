    ...
    <div className="space-y-4">
      <div className="p-3">
        ...
        <p className="text-muted-foreground text-xs">
          HOD Dashboard - {data.user.department}
        </p>
      </div>
      ...
      <Card>
        ...
        <CardContent className="space-y-2">
          {data.teamUtilization.map((m) => (
            <div key={m.id} className="flex justify-between border rounded p-3">
              <span>{m.name}</span>
              <span className="text-sm text-muted-foreground">{m.trips} trips</span>
            </div>
          ))}
        </CardContent>
      </Card>
